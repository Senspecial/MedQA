...
from src.utils.text_utils import TextNormalizer, TextSegmenter
...
class QueryProcessor:
    ...
    def clean_query(self, query: str) -> str:
        ...
        # 基本清理
        cleaned = self.normalizer.normalize_text(
            query,
        ...
        if not cleaned or not cleaned.strip():
            logger.warning(f"清理后查询为空，返回原始查询: {query}")
            return query.strip()
        ...
        if self.remove_punctuation:
            ...
            # 如果移除标点后为空，返回清理后的查询（移除标点之前）
            if not cleaned or not cleaned.strip():
                logger.warning(f"移除标点后查询为空，返回清理后的查询")
                # 重新清理但不移除标点
                cleaned = self.normalizer.normalize_text(
                    query,
                    lowercase=False,
        ...
        if len(cleaned) > self.max_query_length:
            logger.warning(f"查询过长，已截断: {len(cleaned)} -> {self.max_query_length}")
    ...
    def process_query(self, query: str) -> str:
        ...
        # 如果所有词都被过滤掉，返回清理后的查询
        if not tokens:
            logger.warning(f"分词后所有词被过滤，返回清理后的查询: {cleaned_query}")
        ...
        if not processed_query or not processed_query.strip():
            logger.warning(f"组合后查询为空，返回清理后的查询: {cleaned_query}")
            return cleaned_query
        
        # 查询扩展
        if self.use_query_expansion:
            expanded_query = self.expand_query(processed_query, tokens)
            # 如果扩展后的查询不为空，使用扩展后的查询
